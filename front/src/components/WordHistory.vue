<template>
  <section
    class="flex h-full max-h-56 flex-col rounded-2xl border border-slate-800/80 bg-slate-950/80 p-4 text-xs text-slate-200 shadow-lg backdrop-blur"
    aria-label="Historique des mots"
  >
    <p class="mb-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">
      Historique
    </p>
    <ul v-if="wordHistory.length" class="max-h-40 space-y-2 overflow-y-auto pr-1 sm:max-h-48">
      <li
